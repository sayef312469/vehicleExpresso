    ...
    if (!month || month == 'ALL') {
      ...
      d[1] = await runQuery(
        `select count(c.SERVICE_ID) AS "Basic & Premium"
            ...
            c.SERVICE_ID=t.SERVICE_ID AND 
            EXTRACT(YEAR FROM t.SERVICE_DATE)=:year AND 
            l.MAINTENANCE_CATEGORY='Premium'`,
        { year },
      )
...
const shortUser = async (req, res) => {
  ...
  try {
    ...
    if (exists.length === 0) {
      await runQuery(
        `insert into vehicle_info (VEHICLENO, VEHICLE_OWNER, VEHICLETYPE, VEHICLE_MODEL, VEHICLE_COMPANY, VEHICLE_COLOR)
    ...
    const data = await runQueryOutBinds(
      `insert into care_transac(MECHANIC_NAME,SERVICE_TYPE,SERVICING_COST)
    ...
    await runQuery(
      `insert into Shorttermcare (SHORTTERM_ID,LABOR_HOURS,REPAIR,WASH,COMPLETED)
    ...
    await runQuery(
      `INSERT INTO takes_care (SERVICE_ID, VEHICLENO,SERVICE_DATE)
...
const longUser = async (req, res) => {
  ...
  try {
    const exists = await runQuery(
      `select * from vehicle_info 
        where VEHICLENO = :vehicleno AND VEHICLE_OWNER = :vehicleowner`,
      { vehicleno, vehicleowner },
    )
    if (exists.length === 0) {
    ...
    const service_date = date
    await runQuery(
      `insert into Longtermcare (LONGTERM_ID,MAINTENANCE_CATEGORY,INSURANCE_PROVIDER,INSURANCE_EXP_DATE,ODOMETER_READING,FINAL_DATE)
...
const tableFetch = async (req, res) => {
  try {
    const data = await runQuery(
      `select u.name,vi.vehicleno,ct.service_id,tc.service_date,
...
const updateTable = async (req, res) => {
  ...
  try {
    ...
    await runQuery(
      `
        ...
        BEGIN
            ...
            WHERE shortterm_id = :service_id;
        END;`,
      { service_id, repairCost, washCost, labourHour, status, service_id },
    )
    ...
    await runQuery(
      ...
      { mechanic, servicing_cost, service_id },
    )

    const data1 = await runQuery(
      `select * from care_transac
        where service_id=:service_id`,
      { service_id },
    )
    const data2 = await runQuery(
      `select * from shorttermcare
        where shortterm_id=:service_id`,
      { service_id },
    )
