...
const LineCharts = () => {
  ...
  useEffect(() => {
    if (!error && linedata.monthCost && Array.isArray(linedata.monthCost)) {
      const LineLabel = linedata.monthCost.map((line) => {
        return Object.keys(line)[0]
      })
      const LineData = linedata.monthCost.map((line) => {
      ...
      const LineLabel = linedata.dayCost.map((line) => {
        return Object.keys(line)[0]
      })
      const LineData = linedata.dayCost.map((line) => {
        return Object.values(line)[0]
      })
      setLabels(LineLabel)
      setValues(LineData)
